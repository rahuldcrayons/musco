<x-layouts.app>
    ...
    @push('meta')
        ...
        <?php
            ...
            $reelThumb = $reel['thumbnail_url'] ?: asset('images/musco-banner.png');
    ...
    <div class="bg-[#f8f6f3] min-h-screen">
        ...
        <div class="container mx-auto px-4 py-4">
            <nav class="text-sm text-[#555555]">
                <a href="{{ route('home') }}" class="hover:text-[#c29958]">Home</a>
                <span class="mx-1">/</span>
                <a href="{{ route('reels.index') }}" class="hover:text-[#c29958]">Instagram</a>
                <span class="mx-1">/</span>
                <span class="text-[#222222]">{{ Str::limit($reel['caption'], 30) ?: $reel['shortcode'] }}</span>
            </nav>
        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div class="sticky top-24">
                        ...
                        <div class="flex items-center justify-between mt-4">
                            <a href="{{ $reel['permalink'] }}" target="_blank" rel="noopener"
                               class="inline-flex items-center gap-2 text-sm text-[#555555] hover:text-[#c29958] transition-colors">
                ...
                <div class="lg:col-span-7">
                    <div class="bg-white rounded-2xl p-6 lg:p-8 shadow-sm">
                        <p class="text-xs text-[#555555] mb-3">{{ \Carbon\Carbon::parse($reel['timestamp'])->diffForHumans() }}</p>

                        @if($reel['caption'])
                            <p class="text-[#222222] text-sm leading-relaxed mb-6 whitespace-pre-line">{{ $reel['caption'] }}</p>
                        @endif

                        {{-- Shop Now CTA --}}
                        <div class="border-t border-[#efefef] pt-6 mt-6">
                            <h3 class="text-lg font-semibold text-[#222222] mb-3">Shop Our Products</h3>
                            <p class="text-sm text-[#555555] mb-4">Love what you see? Browse our collection.</p>
                            <a href="{{ route('products.index') }}"
                               class="inline-flex items-center gap-2 px-6 py-2.5 bg-[#B76E79] text-white rounded-full font-semibold text-sm hover:bg-[#222222] transition-colors">
                    ...
                    @if(count($moreReels))
                        <div class="mt-8">
                            <h3 class="text-lg font-semibold text-[#222222] mb-4">More from Instagram</h3>
                            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                                @foreach($moreReels as $more)
                                    ...
                                    <a href="{{ route('reels.show', $more['shortcode']) }}" class="group block">
                                        ...
                                        <p class="mt-2 text-xs text-[#555555] line-clamp-2">{{ Str::limit($more['caption'], 50) }}</p>
                                    </a>
                                @endforeach
                            </div>
                        </div>
