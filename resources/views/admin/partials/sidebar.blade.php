...
<!-- Sidebar -->
<aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-30 w-60 text-white transform transition-transform duration-200 ease-in-out lg:translate-x-0 lg:static lg:inset-0 flex flex-col" style="background:#1a1a1a">

    @php $user = auth('admin')->user(); @endphp

    <!-- Logo -->
    <div class="flex items-center h-14 px-4" style="border-bottom:1px solid #333">
    ...
    <!-- Navigation -->
    <nav class="flex-1 overflow-y-auto px-2 pt-2 pb-4 text-sm" style="scrollbar-width:thin;scrollbar-color:#333 transparent">

        @php
            // Helper: check if route matches
            $isActive = fn($patterns) => request()->routeIs(...(array)$patterns);
            $activeStyle = 'style="background:#303030;color:#fff"';
            $linkClass = 'flex items-center gap-3 px-2 py-1.5 rounded-lg mb-px';
            $activeClass = 'text-white';
            $normalClass = 'text-neutral-400 hover:text-white';
        ...
        <div x-data="{ open: {{ $isActive(['admin.orders.*','admin.returns.*','admin.credit-notes.*']) ? 'true' : 'false' }} }">
            <button @click="open = !open" class="{{ $linkClass }} w-full text-left {{ $isActive(['admin.orders.*','admin.returns.*','admin.credit-notes.*']) ? $activeClass : $normalClass }}" {!! $isActive(['admin.orders.*','admin.returns.*','admin.credit-notes.*']) ? $activeStyle : '' !!}>
                ...
                Orders
                @if($pendingOrders > 0)
                <span class="ml-auto text-xs rounded-full px-1.5" style="background:#444;color:#aaa">{{ $pendingOrders }}</span>
                @endif
            </button>
            <div x-show="open" x-cloak class="ml-8 space-y-px mb-1">
                <a href="{{ route('admin.orders.index') }}" class="{{ $subClass }} {{ $isActive('admin.orders.*') ? 'text-white' : 'text-neutral-500 hover:text-white' }}">All Orders</a>
                <a href="{{ route('admin.returns.index') }}" class="{{ $subClass }} {{ $isActive('admin.returns.*') ? 'text-white' : 'text-neutral-500 hover:text-white' }}">Returns</a>
                <a href="{{ route('admin.credit-notes.index') }}" class="{{ $subClass }} {{ $isActive('admin.credit-notes.*') ? 'text-white' : 'text-neutral-500 hover:text-white' }}">Credit Notes</a>
            </div>
        </div>
        @endif
        ...
        <div x-data="{ open: {{ $isActive(['admin.products.*','admin.categories.*','admin.brands.*','admin.attributes.*','admin.inventory.*']) ? 'true' : 'false' }} }">
            ...
            <div x-show="open" x-cloak class="ml-8 space-y-px mb-1">
                <a href="{{ route('admin.products.index') }}" class="{{ $subClass }} {{ $isActive('admin.products.*') ? 'text-white' : 'text-neutral-500 hover:text-white' }}">All Products</a>
                <a href="{{ route('admin.categories.index') }}" class="{{ $subClass }} {{ $isActive('admin.categories.*') ? 'text-white' : 'text-neutral-500 hover:text-white' }}">Collections</a>
                <a href="{{ route('admin.inventory.index') }}" class="{{ $subClass }} {{ $isActive('admin.inventory.*') ? 'text-white' : 'text-neutral-500 hover:text-white' }}">Inventory</a>
                <a href="{{ route('admin.brands.index') }}" class="{{ $subClass }} {{ $isActive('admin.brands.*') ? 'text-white' : 'text-neutral-500 hover:text-white' }}">Brands</a>
                <a href="{{ route('admin.attributes.index') }}" class="{{ $subClass }} {{ $isActive('admin.attributes.*') ? 'text-white' : 'text-neutral-500 hover:text-white' }}">Attributes</a>
            </div>
        </div>
        @endif
        ...
        <div x-data="{ open: {{ $isActive(['admin.pages.*','admin.blog-posts.*','admin.reviews.*']) ? 'true' : 'false' }} }">
            ...
            <div x-show="open" x-cloak class="ml-8 space-y-px mb-1">
                <a href="{{ route('admin.pages.index') }}" class="{{ $subClass }} {{ $isActive('admin.pages.*') ? 'text-white' : 'text-neutral-500 hover:text-white' }}">Pages</a>
                <a href="{{ route('admin.blog-posts.index') }}" class="{{ $subClass }} {{ $isActive('admin.blog-posts.*') ? 'text-white' : 'text-neutral-500 hover:text-white' }}">Blog Posts</a>
                <a href="{{ route('admin.reviews.index') }}" class="{{ $subClass }} {{ $isActive('admin.reviews.*') ? 'text-white' : 'text-neutral-500 hover:text-white' }}">Reviews</a>
            </div>
        </div>
        @endif
        ...
        <div x-data="{ open: {{ $isActive(['admin.enquiries.*','admin.support-tickets.*','admin.fraud.*']) ? 'true' : 'false' }} }">
            ...
            <div x-show="open" x-cloak class="ml-8 space-y-px mb-1">
                <a href="{{ route('admin.enquiries.index') }}" class="{{ $subClass }} {{ $isActive('admin.enquiries.*') ? 'text-white' : 'text-neutral-500 hover:text-white' }}">Enquiries</a>
                <a href="{{ route('admin.support-tickets.index') }}" class="{{ $subClass }} {{ $isActive('admin.support-tickets.*') ? 'text-white' : 'text-neutral-500 hover:text-white' }}">Tickets</a>
                <a href="{{ route('admin.fraud.index') }}" class="{{ $subClass }} {{ $isActive('admin.fraud.*') ? 'text-white' : 'text-neutral-500 hover:text-white' }}">Fraud Review</a>
            </div>
        </div>
        ...
        <div class="mt-2 pt-2" style="border-top:1px solid #333">
            ...
            <a href="{{ route('admin.sellers.index') }}" class="{{ $subClass }} mb-px {{ $isActive('admin.sellers.*') ? 'text-white' : 'text-neutral-500 hover:text-white' }}">Sellers</a>
            @endif
            @if($user->canAccessSection('staff'))
            <a href="{{ route('admin.staff.index') }}" class="{{ $subClass }} mb-px {{ $isActive('admin.staff.*') ? 'text-white' : 'text-neutral-500 hover:text-white' }}">Staff</a>
            @endif
            @if($user->canAccessSection('delivery_partners'))
            <a href="{{ route('admin.delivery-partners.index') }}" class="{{ $subClass }} mb-px {{ $isActive('admin.delivery-partners.*') ? 'text-white' : 'text-neutral-500 hover:text-white' }}">Delivery Partners</a>
            @endif
        </div>
        @endif
    </nav>
    ...
    <div class="px-2 pb-3 pt-1" style="border-top:1px solid #333">
