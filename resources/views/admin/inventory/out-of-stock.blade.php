<x-layouts.admin>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <div class="flex items-center gap-2 text-sm text-neutral-600 mb-1">
                ...
                <span class="text-neutral-900">Out of Stock</span>
            </div>
            <h1 class="text-2xl font-bold text-neutral-900">Out of Stock Products</h1>
            <p class="text-neutral-600">Products with zero or negative stock</p>
        </div>
    ...
    @if($products->total() > 0)
        <div class="card px-5 py-4 mb-6 border-l-4 border-error-400 bg-error-50">
            <div class="flex items-center gap-3">
                <svg class="w-5 h-5 text-error-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                </svg>
                <p class="text-sm font-medium text-error-800">
                    <span class="font-bold">{{ $products->total() }}</span> {{ Str::plural('product', $products->total()) }} out of stock. These are unavailable to customers.
                </p>
            </div>
        </div>
    @endif
                    ...
                    @forelse($products as $product)
                        <tr class="hover:bg-neutral-50 transition-colors">
                            ...
                            <td class="px-4 py-3 text-center">
                                <span class="badge badge-error">{{ $product->stock_quantity }}</span>
                            </td>
                        ...
                        <tr>
                            <td colspan="4" class="px-4 py-16 text-center">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <p class="text-sm font-medium text-neutral-900">All products are in stock!</p>
                                    <p class="text-xs text-neutral-600">No products have zero stock right now.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    <div x-data="{ open: false, productId: null, productName: '', currentStock: 0 }"
        ...
        <div class="relative bg-white rounded-xl shadow-xl w-full max-w-md mx-4" x-transition>
            ...
            <div class="px-6 py-3 bg-error-50 border-b border-error-100">
                <div class="flex items-center justify-between">
                    <span class="text-xs text-error-700">Current Stock</span>
