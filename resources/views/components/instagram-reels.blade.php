{{-- Shoppable Instagram Reels Carousel --}}
@php
    $reels = app(\App\Services\InstagramReelsService::class)->getLatestReels(10);
    $igHandle = config('services.instagram.handle');

    // Get some featured/bestseller products to pair with reels
    ...
    <div class="container mx-auto px-4">
        ...
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="section-title" style="margin:0;font-size:22px;">Shop Our Reels</h2>
                    <p class="text-xs text-[#565959] mt-0.5">Watch, love, and shop instantly</p>
                </div>
            </div>
            <a href="{{ route('reels.index') }}" class="view-all-link" style="display:inline-flex;align-items:center;gap:6px;">
                View All
        ...
        {{-- Reels Carousel --}}
        <div class="ig-shop-carousel" x-data="{ scrollEl: null }" x-init="scrollEl = $refs.track">
            <div class="ig-shop-track" x-ref="track">
                @foreach($reels as $index => $reel)
                    @php $product = $shopProducts[$index] ?? null; @endphp
                    <div class="ig-shop-item">
                        {{-- Video Thumbnail --}}
                        <a href="{{ route('reels.show', $reel['shortcode']) }}" class="ig-shop-thumb group">
                            @if($reel['thumbnail_url'])
                                <img src="{{ $reel['thumbnail_url'] }}" alt="{{ Str::limit($reel['caption'], 60) }}" loading="lazy">
                            @else
                                <div class="ig-shop-placeholder"></div>
                            @endif

                            {{-- Play overlay --}}
                            <div class="ig-shop-overlay">
                                <div class="ig-shop-play">
                            ...
                            {{-- Shop tag at bottom of video --}}
                            @if($product)
                                <div class="ig-shop-tag">
                        ...
                        @if($product)
                            <div class="ig-shop-product">
                                <a href="{{ route('products.show', $product->slug) }}" class="ig-shop-product-link">
                                    <p class="ig-shop-product-name">{{ Str::limit($product->name, 40) }}</p>
                                    <div class="ig-shop-product-price">
                                        <span class="ig-shop-price-current">₹{{ number_format($product->price, 0) }}</span>
                                        @if($product->mrp > $product->price)
                                            <span class="ig-shop-price-mrp">₹{{ number_format($product->mrp, 0) }}</span>
                                            <span class="ig-shop-price-off">{{ $product->discount_percentage }}% off</span>
                                        @endif
                                    </div>
                                </a>
                                <a href="{{ route('products.show', $product->slug) }}"
                                   class="ig-shop-buy-btn">
                                    Buy
                                </a>
                            </div>
                        @else
                            {{-- Caption fallback --}}
                            @if($reel['caption'])
                                <p class="ig-shop-caption">{{ Str::limit($reel['caption'], 45) }}</p>
                            @endif
                        @endif
                    </div>
                @endforeach
        ...
        @if($igHandle)
            <div class="text-center mt-6">
                <a href="https://instagram.com/{{ ltrim($igHandle, '@') }}" target="_blank" rel="noopener"
...
<style>
    ...
    .ig-shop-thumb:hover img { transform: scale(1.05); }

    .ig-shop-placeholder {
        width: 100%; height: 100%;
        background: linear-gradient(135deg, #205258, #1b454a);
    }
    ...
    .ig-shop-thumb:hover .ig-shop-overlay { opacity: 1; }
    ...
    .ig-shop-thumb:hover .ig-shop-play { transform: scale(1.1); }
    ...
    .ig-shop-tag {
        ...
        font-size: 12px; font-weight: 700; color: #205258;
        opacity: 0; transform: translateY(6px);
        transition: all 0.3s;
    }
    .ig-shop-thumb:hover .ig-shop-tag { opacity: 1; transform: translateY(0); }
    ...
    .ig-shop-product-link {
        text-decoration: none; color: inherit; flex: 1; min-width: 0;
    }
    .ig-shop-product-name {
        font-size: 12px; font-weight: 500; color: #0F1111; margin: 0 0 3px;
        line-height: 1.3;
        display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;
    }
    .ig-shop-product-link:hover .ig-shop-product-name { color: #205258; }
    .ig-shop-product-price {
        display: flex; align-items: baseline; gap: 4px; flex-wrap: wrap;
    }
    .ig-shop-price-current {
        font-size: 14px; font-weight: 700; color: #0F1111;
    }
    .ig-shop-price-mrp {
        font-size: 11px; color: #999; text-decoration: line-through;
    }
    .ig-shop-price-off {
        font-size: 10px; font-weight: 600; color: #CC0C39;
    }
    .ig-shop-buy-btn {
        flex-shrink: 0; margin-top: 2px;
        padding: 5px 14px; background: #205258; color: #fff;
        border-radius: 20px; font-size: 12px; font-weight: 600;
        text-decoration: none; transition: background 0.2s; white-space: nowrap;
    }
    .ig-shop-buy-btn:hover { background: #1b454a; }

    .ig-shop-caption {
        margin: 8px 0 0; font-size: 12px; color: #565959;
    ...
    @media (max-width: 767px) {
        ...
        .ig-shop-tag { padding: 6px 10px; font-size: 11px; }
        .ig-shop-buy-btn { padding: 4px 10px; font-size: 11px; }
    }
    @media (max-width: 480px) {
        .ig-shop-item { flex: 0 0 140px; }
        .ig-shop-product-name { font-size: 11px; }
        .ig-shop-price-current { font-size: 13px; }
    }
</style>
@endif
