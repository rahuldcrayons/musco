{{-- Exit-Intent Popup — First-time visitor discount --}}
@guest
@php
    $exitDiscount = \App\Models\Setting::get('exit_popup_discount', '10');
    $exitHeadline = \App\Models\Setting::get('exit_popup_headline', "Wait! Don't leave yet");
    $exitSubheading = \App\Models\Setting::get('exit_popup_subheading', "Get <span class=\"font-bold text-[#CC0C39]\">{$exitDiscount}% OFF</span> your first order. Enter your email and we'll send you the discount code instantly.");
    $exitButtonText = \App\Models\Setting::get('exit_popup_button_text', "GET {$exitDiscount}% OFF");
    $exitSuccessMsg = \App\Models\Setting::get('exit_popup_success_message', "Check your email! Your {$exitDiscount}% discount code is on its way.");
@endphp
<div x-data="exitIntent()" x-show="show" x-cloak
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden"
        ...
        <button @click="dismiss()" class="absolute top-3 right-3 z-10 w-8 h-8 flex items-center justify-center rounded-full bg-black/10 hover:bg-black/20 transition-colors">
            <svg class="w-4 h-4 text-neutral-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
        </button>
        ...
        <div class="p-6 sm:p-8 text-center">
            ...
            <p class="text-sm text-[#555555] mb-5">{!! $exitSubheading !!}</p>
