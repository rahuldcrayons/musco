...
<p style="font-size:14px;color:#333;margin:0 0 16px;">Hi <strong>{{ $order->user->first_name ?? $order->guest_name ?? 'there' }}</strong>, thanks for shopping with {{ \App\Models\Setting::get('site_name', 'Trendymus') }}!</p>
...
<strong style="font-size:14px;color:#202a40;">#{{ $order->order_number }}</strong>
</td>
...
<strong style="font-size:14px;color:#202a40;">{{ $order->created_at->format('d M Y') }}</strong>
</td>
</tr>
<tr>
...
<strong style="font-size:18px;color:#202a40;">{{ format_price($order->total) }}</strong>
</td>
</tr>
</table>

<p style="font-size:13px;font-weight:bold;color:#202a40;margin:0 0 8px;padding-bottom:6px;border-bottom:2px solid #202a40;">ITEMS</p>
...
<a href="{{ url('/account/orders') }}" style="background:#202a40;color:#f0ebe6;text-decoration:none;padding:12px 32px;border-radius:6px;font-size:14px;font-weight:600;display:inline-block;">Track Your Order</a>
</div>

<p style="font-size:11px;color:#999;text-align:center;margin:16px 0 0;">Need help? Reply to this email or WhatsApp us at {{ \App\Models\Setting::get('site_phone', '+44 7354 567705') }}</p>
...
<a href="{{ url('/account/orders') }}" style="color:#f0ebe6;font-size:12px;text-decoration:none;margin:0 8px;">Orders</a>
<a href="{{ url('/contact') }}" style="color:#f0ebe6;font-size:12px;text-decoration:none;margin:0 8px;">Contact</a>
